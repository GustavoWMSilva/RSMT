...
class BatchProcessData(torch.nn.Module):#(B,T,J,dim)
    ...
    def forward(self,global_rotations,global_positions):
        ...
        dir_vec = global_positions[..., 5:6, :] - global_positions[..., 1:2, :]
        
        device = torch.device("cpu")
        up_vec = torch.tensor([0, 1, 0], dtype=dir_vec.dtype, device=device)
        up_vec = up_vec.view(*([1] * (dir_vec.dim() - 1)), 3)  # Adiciona dimensões extras
        ref_vector = torch.cross(dir_vec, up_vec, dim=-1)
...
class BatchRotateYCenterXZ(torch.nn.Module):
    ...
    def forward(self,global_positions,global_quats,ref_frame_id):
        ...
        dir_vec = global_positions[..., 5:6, :] - global_positions[..., 1:2, :]
        device = torch.device("cpu")
        up_vec = torch.tensor([0, 1, 0], dtype=dir_vec.dtype, device="cpu")
        up_vec = up_vec.view(*([1] * (dir_vec.dim() - 1)), 3)  # Adiciona dimensões extras
