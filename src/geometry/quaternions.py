...
def from_to_1_0_0(v_from):
    ...
    v_to_unit = torch.tensor(np.array([[1, 0, 0]]), dtype=v_from.dtype, device=device).expand(v_from.shape)
    y = torch.tensor(np.array([[0,1,0]]),dtype=v_from.dtype,device=device).expand(v_from.shape)
    z_to_unit = torch.tensor(np.array([[0,0,1]]),dtype=v_from.dtype,device=device).expand(v_from.shape)
